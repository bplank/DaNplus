...
class MachampUniversalReader(DatasetReader):
    def __init__(self,
        ...
        self._start_token, self._end_token = self._target_tokenizer.tokenize(
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        for dataset in self.datasets:
            ...
            for task in self.datasets[dataset]['tasks']:
                ...
                if self.datasets[dataset]['tasks'][task]['task_type'] == 'unsupervised':
                    num_mlm += 1
            ...
            if num_mlm not in [0, num_tasks]:
                logger.error('A dataset can only consists of 0 unsupervised tasks or all')
    ...
    def read_sequence(self, dataset, path, is_train, max_sents):
        ...
        for sent, full_data in seqs2data(path, self.do_lowercase):
            ...
            data.append(self.text_to_instance(sent_tasks, full_data, col_idxs, is_train, task2type, dataset))
        return data
    ...
    def text_to_instance(self,  # type: ignore
        ...
        for task in sent_tasks:
            if task == 'tokens':
                ...
                fields[task + '_target'] = TextField(sent_tasks[task], self._target_token_indexers)
                fields[task + '_target_words'] = SequenceLabelField([str(x) for x in sent_tasks[task]],
                                                                    fields[task + '_target'],
                                                                    label_namespace=task + "_target_words")
