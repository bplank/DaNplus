...
class BucketBatchSampler(BatchSampler):
    ...
    def __init__(
        ...
        self.first = True
        self.maximum_samples_per_batch = maximum_samples_per_batch
    ...
    def __iter__(self) -> Iterable[List[int]]:
        ...
        for dataset_indices, dataset_instances in self.group_by_dataset(self.data_source.instances):
            ...
            sorted_indices, _ = self._argsort_by_padding(dataset_instances, dataset_indices)
            dataset_batches = []
        ...
        if self.sampling_smoothing != 1.0 and is_train:
            ...
            for size in sizes:
                ...
                newSizes.append(int(size * prob))

            # collect all batches
            this_epoch_all_batches = []
            for dataset_idx in range(len(all_batches)):
                newSize = newSizes[dataset_idx]
                random.shuffle(all_batches[dataset_idx])
                while newSize > len(all_batches[dataset_idx]):
                    all_batches[dataset_idx] += all_batches[dataset_idx]
                this_epoch_all_batches += all_batches[dataset_idx][:newSize]
